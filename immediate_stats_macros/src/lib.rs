...
pub fn stat_container_derive(item: TokenStream) -> TokenStream {
    let tree: DeriveInput = syn::parse(item)
        .expect("A valid TokenStream");
        ...
        Data::Struct(s) =>  {
            for field in s.fields {
                ...
                for attr in field.attrs {
                    ...
                    let Ok(attr_ident) = path.require_ident() else { continue };
                    let Ok(attr_type) = AttrType::try_from(attr_ident) else { continue };
